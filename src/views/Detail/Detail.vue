...
export default {
	...
	methods: {
		// 点击切换售卖属性：排他操作
		changeActive (valueObj, arr) {
			arr.forEach(item => {
				item.isChecked = 0
			});
			valueObj.isChecked = 1
		},
		changeSkuNum (event) {
			// 对用户输入的文本乘以1，数字相乘还是数字，非数字相乘会得到NaN
		...
		async addToCart () {
			// 1.派发action，将产品信息通知服务器，派发结果是一个Promise
			try {
				...
				// 2.服务器存储成功，进行路由跳转
				// 2.1 skuNum通过query进行传递
				this.$router.push({ name: 'addcartsuccess', query: { skuNum: this.skuNum } })
				// 2.2 skuInfo通过会话存储进行传递，只能传递字符串形式，因为通过JSON.stringify()转为json格式
